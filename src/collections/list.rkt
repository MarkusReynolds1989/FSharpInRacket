...
; Returns a new list that contains the elements of the first list followed by the elements
; of the second list.
(define (append list-one list-two)
  (if (null? list-one) list-two (cons (car list-one) (append (cdr list-one) list-two))))

; Returns the avereage of the values in a non-empty list.
(define (average source)
  (/ (apply + source) (length source)))

; Returns the average of values in a list generated by applying a
; function to each element of the list.
(define (average-by projection source)
  (average (map projection source)))

; May not need choose, revisit this.

; TODO: Implement
; Divids the list into lists with positive number of at most
; chunk-size elements. Returns an empty list when input list is empty.
(define (chunk-by-size chunk-size source)
  (if (is-empty? source) '() (raise "Not implemented.")))

; For each element of the list, applies the given function. Cocatonates all the results
; and return the combined list.
; (: collect (All (T U) (-> (-> T (Listof U)) (Listof T) (Listof U)))
